...
constraints_4   = -q*(q+1)         *   u(3)*(x(3) - x(1));
constraints_5   = -q*(q+1)         *   u(1)*(x(1) - x(2));

%% LHS
LHS =        q*(q+1)/2    * (y(1) + u(1))^2 + q     * (y(1) + u(1))*(x(1) - x0);
LHS = LHS -(q+1)*(q+2)/2  * (y(3) + u(3))^2 - (q+1) * (y(3) + u(3))*(x(3) - x0);
...
%% RHS
RHS = q*(q+1)/2*(x(2) - x(1) + y(1) + u(1) + 1/(q+1)*(x(1) - x0))^2;
RHS = RHS + (1-p)*q*(q+1)/(2*p)*(y(2) - y(3))^2;
RHS = RHS + (q+1)*(y(2) - y(3))*(y(3)+u(3)); 
