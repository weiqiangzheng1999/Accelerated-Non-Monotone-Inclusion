...
u(1) = (x(1) + 1/(q+1)*(x0 - x(1)) - (1 - 1/(q+1))*(1 + 2*r)*y(1) - x(2))/((1 - 1/(q+1))*(1 + 2*r));
u(3) = x(1) + 1/(q+1)*(x0 - x(1)) - y(2) - (1 - 1/(q+1))*2*r*(y(1)+u(1)) - x(3); 
...
LHS = LHS - q*(q+1)*((y(1)+u(1) - y(3)-u(3))*(x(1) - x(3))-r*(y(1)+u(1) - y(3) - u(3))^2); 

%% RHS
RHS = (x(2) - x(3))^2 * (1-p) * (q + 1)^2/2;
